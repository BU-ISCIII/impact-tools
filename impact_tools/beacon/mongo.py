...
def mongo_upsert_duo_filtering_terms(
    database: Database,
    terms: list[dict[str, Any]],
) -> int:
    """Upsert dataset-scoped DUO terms into db.filtering_terms.

    These are intentionally limited to DUO data use conditions generated by
    `impact-tools beacon ingest dataset`; this does not run the generic Beacon
    filtering-terms extractor.
    """
    ...
    for term in terms:
        ...
        if not isinstance(term_id, str) or not term_id:
            raise ValueError(f"Invalid DUO filtering term id: {term!r}")

        if not isinstance(label, str) or not label:
            raise ValueError(f"Invalid DUO filtering term label: {term!r}")
        ...
        result = database.filtering_terms.update_one(
            {"id": term_id},
            {"$set": document},
            upsert=True,
        )
